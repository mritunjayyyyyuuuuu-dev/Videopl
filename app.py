...
def auto_discover_videos():
    ...
    while True:
        for domain in DOMAINS:
            # ========================================================
            # STEP 1: Quick DB check (Kis domain ka kya status hai)
            # ========================================================
            conn = video_db_pool.getconn()
            cur = conn.cursor()
            try:
            ...
            # ========================================================
            # STEP 2: Network Scanning (Without holding DB Connection)
            # ========================================================
            if should_scan:
                ...
                while True:
                    ...
                    # --- INTERNET CHECK (No Database connection active here) ---
                    try:
                        ...
                        consecutive_misses += 1  # Network drop ko bhi miss me count karenge

                    # ========================================================
                    # STEP 3: Quick DB Write (Open -> Save -> Close)
                    # ========================================================
                    ...
                    try:
                        if status_success:
                            ...
                            if not status_success: # Only add to misses if it wasn't a network error tracked above
                        ...
                        save_conn.rollback() # VERY IMPORTANT: Rollback broken transactions
                ...
                # ========================================================
                # STEP 4: Update Domain Last Scan Logs
                # ========================================================
                ...
                try:
                    ...
                    log_conn.rollback()
                    print(f"[Crawler Log Update Error] {log_e}")
                finally:
                    log_cur.close()
