...
def run_db_migrations():
    ...
    try:
        ...
        # 3. Create table for tracking Domain scan status
        cur.execute("""
        ...
        # 4. Migrate Old Data (Assign 15-char IDs to old videos and set default domain)
...
def auto_discover_videos():
    """Smarter Crawler: Multiple domains, smart batching, 24h checks"""
    while True:
        for domain in DOMAINS:
            conn = video_db_pool.getconn()
            cur = conn.cursor()
            try:
                # Get last scan info for this domain
                ...
                if not scan_log:
                    ...
                    # Agar 24 ghante (86400 seconds) se zyada ho gaya hai toh scan karo
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if head.status_code == 200 and 'video' in head.headers.get('Content-Type', ''):
                                ...
                                consecutive_misses = 0 # Reset misses kyunki video mil gaya
                                last_num = current_num # Update highest successful number
                            else:
                                consecutive_misses += 1
                                # Keep track in DB as Not Found so we don't try again randomly
                                cur.execute("""
                                    INSERT INTO video_metadata_scan (video_num, video_url, size_mb, status, is_processed, video_id, domain)
                        ...
                        # ================= STOPPING LOGIC =================
                        if is_first_scan:
                            # First time: Stop agar ek baar video milne ke baad 200 lagatar na mile, ya 1500 cross kar jaye
                            if (found_any_video and consecutive_misses >= 200) or current_num >= 1500:
                                print(f"🛑 [Crawler] First scan stopped for {domain}. Last Found: {last_num}. (Misses: {consecutive_misses})")
                                break
                        else:
                            # Routine scan: Agar lagatar 50 na mile (matlab naye upload nahi hue), toh ruk jao
                            if consecutive_misses >= 50:
                                print(f"🛑 [Crawler] Routine scan finished for {domain}. Last Found: {last_num}. (Misses: 50)")
                                break

                        current_num += 1
                        time.sleep(0.5) # Avoid hammering the server too fast
                    
                    # Scanning done, update the log time and max number
                    cur.execute("UPDATE domain_scan_logs SET last_scan_time = %s, last_video_num = %s WHERE domain = %s;", (datetime.now(), last_num, domain))
                    conn.commit()

            except Exception as e:
                print(f"[Crawler DB Error for {domain}] {e}")
            finally:
                cur.close()
                video_db_pool.putconn(conn)
        
        # Har 5 minute (300 seconds) me check karega ki kis domain ka 24 hours pura hua hai
...
def startup_event():
    ...
    # Setup aur cache loading ko naye thread me start karo
